#!/usr/local/bin/python3.7  

#UNPACKING - PASSANDO 3 OU + PARAMETROS
def preco_final(preco_bruto,calc_imposto,*params):
    if callable(calc_imposto):
        return preco_bruto + preco_bruto * calc_imposto(*params) 
    else: 
        print("Not callable") #

def imposto_x(importado):
    return 0.22 if importado else 0.13

def imposto_y(explosivo,fator_mult=1):
    return 0.11 * fator_mult if explosivo else 0

if __name__ == "__main__":
    preco_bruto = 134.98
    pfinal = preco_final(preco_bruto,imposto_x,True)
    pfinal = preco_final(pfinal,imposto_y,True,1.5)
    print(pfinal)